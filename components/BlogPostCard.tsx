...
export default function BlogPostCard({ title, slug, summary, className }: Props) {
  ...
  return (
    <CustomLink
      href={`/blog/${slug}`}
      className={cn(
        'transform hover:scale-[1.02] sm:hover:scale-[1.07] transition-all duration-200 ease-in-out rounded-md w-full md:w-1/3 border-4 border-sky-500 dark:border-sky-600',
        className
      )}
    >
